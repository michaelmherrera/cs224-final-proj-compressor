...
# def process_og_gz_neural_varint():
#     with open("her_u_go_kasey.json", "r") as read_file:
#         data = json.load(read_file)
#         og = np.array(data['og'])
#         gz_baseline = np.array(data['gz'])
#         neural_varint = np.array(data['neural'])
#     return og, gz_baseline, neural_varint
...
# neural_arith = process_arith_neural()
# og, gz_baseline, neural_varint = process_og_gz_neural_varint()
# arith_baseline = process_arith_baseline()
# gz = process_gz_1000()

# arith_baseline = og / arith_baseline
# gz_baseline = og / gz_baseline
# gz_baseline, neural_varint, neural_arith, arith_baseline = get_compression_ratio(gz_baseline, neural_varint, og, neural_arith, arith_baseline)

print("GZIP mean, min, max, std: ", np.mean(gz_baseline), np.min(gz_baseline), np.max(gz_baseline), np.std(gz_baseline))
print("Arith baseline mean, min, max, std: ", np.mean(arith_baseline), np.min(arith_baseline), np.max(arith_baseline), np.std(arith_baseline))
print("Neural varint mean, min, max, std: ", np.mean(neural_varint), np.min(neural_varint), np.max(neural_varint), np.std(neural_varint))
print("Neural airth mean, min, max, std: ", np.mean(neural_arith), np.min(neural_arith), np.max(neural_arith), np.std(neural_arith))
